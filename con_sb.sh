#!/bin/bash
#example:
# bash ./con_sb.sh -i i-0debd550b7a747e7f   -a ami-48998d2a  -g S  -t host & > 2>&1 > output_`date +"\%Y\%m\%d"`.log
...
################
# echo "Data Volumes number: ${#DataVolumes[*]}"
# echo "Data Volumes : ${DataVolumes[@]}"
# echo "Data Devices number: ${#DataDevices[*]}"
# echo "Data Devices : ${DataDevices[@]}"
################
...
if  [[ $SourceTags != $a ]];
 then
   ...
   #cmd="aws ec2 create-tags --profile $profile --resources $TargetInstanceId --tags file://$SourceInstanceId.json"
...
cmd="aws ec2 wait instance-running --profile $profile --instance-ids  $TargetInstanceId"
try_function "${cmd}"
